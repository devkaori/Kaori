...
module.exports = async (client) => {
    ...
    setInterval(async function () {
        ...
        const statusText = `Sur ${totalGuilds.reduce((acc, guildCount) => acc + guildCount, 0)} serveurs | ${totalMembers.reduce((acc, memberCount) => acc + memberCount, 0)} membres | ${totalEmotes.reduce((acc, emoteCount) => acc + emoteCount, 0)} emotes`;

        client.user.setPresence({ activities: [{ name: statusText, type: 'WATCHING' }], status: 'online' });
    }, 50000)
}
